...
const source = await readFile(resolve(process.cwd(), "contracts/BaseFocusSprint.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "BaseFocusSprint.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["BaseFocusSprint.sol"]?.BaseFocusSprint;
...
console.log(`Deploying BaseFocusSprint from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_FOCUS_SPRINT_CONTRACT_ADDRESS to this contract address in Vercel.");
